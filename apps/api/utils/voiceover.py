...
def generate_voiceover_huggingface_tts(script, output_path, voice_name="default"):
    ...
    """
    try:
        print("Generating voiceover with Hugging Face TTS...")
    ...
    try:
        ...
        if sys.platform == "darwin":
            ...
            try:
                ...
                if (
                    say_res.returncode != 0
                    or not os.path.exists(aiff_path)
                    or os.path.getsize(aiff_path) == 0
                ):
...
def generate_voiceover_xtts_hf(
    script: str, output_path: str, speaker_wav: str, language: str = "en", force_cpu: bool = True
) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            tts.tts_to_file(
                text=script, file_path=output_path, speaker_wav=speaker_wav, language=language
            )
...
def generate_voiceover(script, voice_id="default", output_path=None, use_elevenlabs=True):
    ...
    speaker_wav = (
        voice_id
        if isinstance(voice_id, str) and os.path.exists(voice_id)
        else os.getenv("VOICE_SAMPLE_PATH", "voice_sample.wav")
    )
...
def get_available_voices():
    ...
    """
    # Hugging Face only
    return {"huggingface": {"Default": "default", "Female": "female", "Male": "male"}}
